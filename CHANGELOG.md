...
> 版本: v2.2.0 | 日期: 2026-09-04

---

## v2.2.0 — 问题需求(Case)模块 (2026-09-04)

- **新模块「问题需求」**: 教师/管理员提交问题与改进需求，全流程跟踪。侧栏新增独立分组（所有登录用户可见）
- **后端**: `issues` 表（case_no=C+YYYYMMDD+3位流水 唯一/标题/问题模块/说明≤500字/images JSON/状态机 pending→processing→done/创建人/创建时间/完成时间）+ `routers/issue.py` 6端点：POST 创建(创建即待处理)、GET 列表(分页+状态/模块/关键词筛选)、GET 详情、PATCH 编辑(创建人仅待处理时 可编辑)/状态流转(仅admin，done 时落 completed_at、退回清空)、DELETE(admin 或创建人删本人待处理)、POST upload-image(png/jpg/webp ≤10MB，魔数嗅探拦截伪装扩展名，落 uploads/issues/ 经 nginx /uploads/ 静态服务)
- **前端**: 列表页 8 列(Case/标题/问题模块/状态/创建人/创建时间/完成时间/操作)+筛选器+分页，**PC 表格 ↔ 手机卡片式自适应**(CSS media query, td::before data-label)；提交页(标题≤200/模块下拉/说明≤500字实时计数/多图上传≤9张·预览·提交前可删除)；详情弹窗(全字段+图片缩略图)+**点击放大 lightbox**；admin 状态流转按钮(开始处理/标记完成/退回待处理)
- **浏览器端到端实测**(agent-browser 模拟用户): 登录→提交带图需求(2图上传→删1张→提交)→列表核对(C20260904002/待处理)→详情/图片放大→状态流转(处理中→已完成+完成时间落库→退回清空)→手机视口卡片布局✓；伪装图片 422 拦截✓；测试数据已清理(issues=0, 测试图片已删)
- 修复: upload-image 的 splitext 元组误用 .pop() 导致 500
...
- **新模块**: 系统设置→「💾 备份与恢复」。程序备份(代码/脚本/文档,排可重建物)/数据备份(PostgreSQL+uploads+题图+.env+教研云凭证)/整体备份;仅存服务器本地 `/opt/ai-learning/backups/`;支持下载/删除
- **恢复**: 独立界面+双确认+恢复前自动安全备份(默认开);异步编排(scripts/restore_helper.sh 经 systemd-run,trap保证服务拉起);flock 全局互斥;白名单防穿越;sha256+manifest 完整性校验;磁盘<2GB拒绝;.env 默认跳过恢复
- **质量流水**: CX-BK-REV 评审 11 项(DR-01 P0 恢复编排自杀等)全部吸收 → CX-BK-DEV 实现(自检修 1 bug) → CX-BK-QA 测试 16 用例(13 过/2 失败/1 跳过)抓出 **BQ-01/P0 db_grants 三层根因**(REASSIGN共享对象 + \gexec元命令-c不可用 + grep前导空格),修复后主Agent回归 data 恢复 **19 步全绿**(db_grants True/属主0异常/health ok)
- 台账: BQ-01 FIXED;BQ-02(P1 失败自动回滚)/BQ-03(P3) OPEN 列下迭代
